...
export function useLockPlugin({ editor, editorValue, plugins, isReadOnly }: params): LockPlugin {
  ...
  const parseLock = () => {
    if (editorValue.value.indexOf("<xiaohou-lock>") > -1) {
      preserveds.value = editorValue.value.match(/<xiaohou-lock>([^]*?)<\/xiaohou-lock>/gim) || [];
      preservedGaps.value =
        editorValue.value.split(/<xiaohou-lock>([^]*?)<\/xiaohou-lock>/gim) || [];
  ...
  function protectInternal() {
    setTimeout(() => {
      ...
      if (
        preservedAnchors.value.some((anchor) => {
          if (
            // 1.仅只读范围起点在选取范围中
            (anchor.start.row > selection.start.row && anchor.start.row < selection.end.row) ||
            ...
            (anchor.start.row > selection.start.row &&
              ...
              anchor.start.column < selection.end.column) ||
            // 2.仅只读范围终点在选取范围中
            (anchor.end.row > selection.start.row && anchor.end.row < selection.end.row) ||
            ...
            // 3.只读范围涵盖选取范围
            // 3.1.起止不在同一行
            (anchor.start.row < selection.start.row && anchor.end.row > selection.end.row) ||
            // 3.2.起在同一行 止可能在同一行
            (anchor.start.row === selection.start.row &&
              anchor.start.column < selection.start.column &&
              (anchor.end.row > selection.end.row ||
  ...
  const protectPreservedBoundary = (evt: KeyboardEvent) => {
    ...
    if (
      preservedAnchors.value.some((anchor) => {
        ...
        if (evt.keyCode === 8) {
          if (
            Math.max(startA - 1, startB) <= Math.min(endA - 1, endB) &&
