...
export function useBlankPlugin({editor,editorValue,plugins,isReadOnly,showLock}: params):BlankPlugin {
  ...
  function parseBlank() {
    if (editorValue.value.indexOf('<xiaohou-blank>') > -1) {
      blanks.value = editorValue.value.match(/<xiaohou-blank>([^]*?)<\/xiaohou-blank>/igm) || [];
  ...
  function protectExternal() {
    setTimeout(() => {
      ...
      if (blankAnchors.value.some((anchor) => {
        if (
          // 0.单行 选取在填空中
  ...
  function spliceBlanks() {
    ...
    for (let i = 0, len = blankAnchors.value.length; i < len; i += 1) {
      code = `${code}${blankGaps.value[2 * i]}<xiaohou-blank>${editor.value.getSession().doc.getTextRange(
        new Range(
